...
def compute_local_stiffness_matrix(
    k, element_id, mesh, hz, Variation=None, id_c=None, ce=None
):
...
def compute_load_vector(H, mesh, q, hz):
    ...
    for element in elements:
        ...
        if element.n1 in neumann_nodes and element.n2 in neumann_nodes:
            ...
            # edge length
            l = np.sqrt(
                (element.n1.x - element.n2.x) ** 2 + (element.n1.y - element.n2.y) ** 2
            )  # length of the edge'
            ...
            l = np.sqrt(
                (element.n2.x - element.n3.x) ** 2 + (element.n2.y - element.n3.y) ** 2
            )
            f_e[0] = 0
            f_e[1] = 0.5 * l * q * hz
            f_e[2] = 0.5 * l * q * hz
            ...
            # edge n3-n1
            l = np.sqrt(
                (element.n3.x - element.n1.x) ** 2 + (element.n3.y - element.n1.y) ** 2
            )
            f_e[0] = 0.5 * l * q * hz
            f_e[1] = 0
            f_e[2] = 0.5 * l * q * hz
