...
class Mesh:
    def __init__(self, N, Variation, L, k, y_neumann, y_dirichlet, hz):
        ...
        self.neumann_nodes, self.dirichlet_nodes, self.neumann_nodes_inside = (
            self.boundary_conditiones(Variation)
        )
        ...
        for j in range(n - 1):
            for i in range(n - 1):
                ...
                self.elements.append(
                    Triangle((j * (n - 1) + i) * 2 + 1, node1, node2, node3, k, hz)
                )  # Lower triangle
                self.elements.append(
                    Triangle((j * (n - 1) + i) * 2 + 2, node2, node4, node3, k, hz)
                )  # Upper triangle
    ...
    def boundary_conditiones(self, Variation):
        ...
        neumann_nodes_inside = [
            node
            for node in self.nodes
            if node.id
            not in ({n.id for n in dirichlet_nodes} | {n.id for n in neumann_nodes})
        ]
